	...
	int iWidth, int iHeight)
{
	...
	m_uiMIDIPlayerID = 0;
	SetRect(&m_rcCamera, 0, 0, m_iWidth, m_iHeight); // YENÝ: Kamera baþlatma
}
...
// HATA ÇÖZÜMÜ: EKSÝK OLAN FONKSÝYON GÖVDELERÝ
void GameEngine::SetCamera(int x, int y)
{
	m_rcCamera.left = x;
	m_rcCamera.top = y;
	m_rcCamera.right = x + m_iWidth;
	m_rcCamera.bottom = y + m_iHeight;
}
...
void GameEngine::DrawSprites(HDC hDC)
{
	...
	for (siSprite = m_vSprites.begin(); siSprite != m_vSprites.end(); siSprite++)
	{
		// Sprite'ýn pozisyonunu kameraya göre ayarla
		RECT pos = (*siSprite)->GetPosition();
		int drawX = pos.left - m_rcCamera.left;
		int drawY = pos.top - m_rcCamera.top;

		// Sadece ekranda görünen sprite'larý çiz
		if (drawX < m_iWidth && drawX + (*siSprite)->GetWidth() > 0 &&
			drawY < m_iHeight && drawY + (*siSprite)->GetHeight() > 0)
		{
			// Sprite::Draw metodunun (HDC hDC, int x, int y) parametrelerini aldýðýndan emin ol
			(*siSprite)->Draw(hDC, drawX, drawY);
		}
	}
}
